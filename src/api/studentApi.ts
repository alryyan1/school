...
export type StudentCollectionResponse = { data: Student[] /* Add pagination if needed */ };


export const StudentApi = {
  create: (student: Omit<Student, "id">) => // Send plain object if no file involved
    axiosClient.post<Student>("/students", student),

  update: (id: number, student: Partial<Omit<Student, 'id' | 'photo'>>) => // Don't send photo path in normal update
    axiosClient.put<Student>(`/students/${id}`, student),

  getAll: () => axiosClient.get<StudentCollectionResponse>("/students"),

  getById: (id: number) => axiosClient.get<StudentResourceResponse>(`/students/${id}`),
  ...
  updatePhoto: (id: number, photo: File) => {
    ...
    return axiosClient.post<StudentResourceResponse>(`/students/${id}/photo`, formData, {
