...
const TeacherList: React.FC = () => {
    ...
    if (loading && filteredTeachers.length === 0) {
        return (
            <div className="container max-w-screen-lg mx-auto py-6 px-4 md:py-8 md:px-6" dir="rtl">
                <div className="flex justify-between items-center mb-6">
                    <Skeleton className="h-10 w-48" /> <Skeleton className="h-10 w-32" />
                </div>
                <Skeleton className="h-12 w-full mb-4" />
                <div className="border rounded-lg">
                    <Table><TableHeader><TableRow>{[...Array(7)].map((_, i) => <TableHead key={i}><Skeleton className="h-5 w-full" /></TableHead>)}</TableRow></TableHeader>
    ...
    return (
        <div className="container max-w-screen-lg mx-auto py-6 px-4 md:py-8 md:px-6" dir="rtl">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <h1 className="text-2xl font-semibold text-foreground">قائمة المدرسين</h1>
                <div className="flex gap-2 w-full sm:w-auto">
                    <Input
                        type="text" placeholder="بحث بالاسم, الرقم الوطني, أو البريد..."
                        value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                        className="max-w-xs w-full"
                    />
                    <Button onClick={() => navigate('/settings/teachers/create')} className="whitespace-nowrap">
            ...
            {error && (
                <Alert variant="destructive" className="mb-4">
                    <AlertCircle className="h-4 w-4" /> <AlertTitle>خطأ</AlertTitle>
                ...
                <Table className="min-w-full">
                    ...
                    <TableBody>
                        {loading && filteredTeachers.length === 0 && (
                            [...Array(5)].map((_, i) => (
                                <TableRow key={`skel-${i}`}>
                                    {[...Array(7)].map((_, j) => <TableCell key={`skel-cell-${j}`}><Skeleton className="h-5 w-full" /></TableCell>)}
                                </TableRow>
                            ))
                        )}
                        {!loading && filteredTeachers.length === 0 ? (
                            ...
                            filteredTeachers.map((teacher) => (
                                <motion.tr key={teacher.id} variants={itemVariants} initial="hidden" animate="visible" className="hover:bg-muted/50 transition-colors">
                                    <TableCell className="text-center">
                                        <Avatar className="h-9 w-9 mx-auto">
                                            <AvatarImage src={teacher.photo_path ?? undefined} alt={teacher.name} />
                                            <AvatarFallback><GraduationCap className="h-4 w-4 text-muted-foreground" /></AvatarFallback>
                                        </Avatar>
                                    </TableCell>
                                    <TableCell className="font-medium">{teacher.name}</TableCell>
                                    <TableCell className="hidden sm:table-cell">{teacher.national_id || '-'}</TableCell>
                                    ...
                                    <TableCell className="text-center">
                                        <Badge variant={teacher.is_active ? "default" : "outline"} className={teacher.is_active ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : ""}>
                                    ...
                                    <TableCell className="text-center">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-[160px]">
                                                ...
                                                <DropdownMenuItem onSelect={() => navigate(`/settings/teachers/${teacher.id}`)}><Eye className="ml-2 h-4 w-4" /> عرض</DropdownMenuItem>
                                                <DropdownMenuItem onSelect={() => navigate(`/settings/teachers/${teacher.id}/edit`)}><Edit3 className="ml-2 h-4 w-4" /> تعديل</DropdownMenuItem>
                                                <DropdownMenuSeparator />
                                                <DropdownMenuItem onSelect={() => handleOpenDeleteDialog(teacher)} className="text-destructive focus:text-destructive focus:bg-destructive/10"><Trash2 className="ml-2 h-4 w-4" /> حذف</DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </TableCell>
                                </motion.tr>
