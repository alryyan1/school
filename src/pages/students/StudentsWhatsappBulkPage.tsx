...
const StudentsWhatsappBulkPage: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((s) =>
      ...
      ((s as any).mother_whatsapp || '').toLowerCase().includes(q) ||
      ((s as any).father_whatsapp || '').toLowerCase().includes(q) ||
  ...
  const getParentRecipientsForStudent = (s: Student): string[] => {
    const mother = ((s as any).mother_whatsapp || '').trim();
    const father = ((s as any).father_whatsapp || '').trim();
    const recipients: string[] = [];
    if (mother) recipients.push(mother);
    if (father) recipients.push(father);
    return recipients;
  };

  const handleSend = async () => {
    const chosen = students.filter((s) => selectedIds.includes(s.id));
    // Build recipients list including both parents per student, sequentially (mother then father)
    const recipients = chosen.flatMap((s) => getParentRecipientsForStudent(s));

    if (recipients.length === 0) {
      enqueueSnackbar('اختر طلاب لديهم رقم واتساب لولي الأمر.', { variant: 'warning' });
      return;
    }
    if (!message.trim()) {
    ...
    try {
      ...
      enqueueSnackbar(`تم جدولة الإرسال لعدد ${res.queued} طالب (كل 30 ثانية).`, { variant: 'success' });
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle>إرسال واتساب جماعي للطلاب</CardTitle>
            {bulkSendId && (
              ...
              <Table className="w-full">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-center">واتساب الأم</TableHead>
                    <TableHead className="text-center">واتساب الأب</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading && (
                    ...
                    return (
                      <TableRow key={s.id}>
                        <TableCell className="text-center">
                          <Checkbox checked={checked} onCheckedChange={(c) => toggleOne(s.id, !!c)} />
                        </TableCell>
                        <TableCell className="text-center">{s.name}</TableCell>
                        <TableCell className="text-center">{(s as any).mother_whatsapp || '-'}</TableCell>
                        <TableCell className="text-center">{(s as any).father_whatsapp || '-'}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
...
export default StudentsWhatsappBulkPage;
