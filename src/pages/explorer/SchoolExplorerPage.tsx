...
import { Building, DoorOpen, ArrowLeft } from 'lucide-react'; // Example icons
...
const SchoolExplorerPage: React.FC = () => {
    ...
    // Render cards view for mobile and tablet
    const renderCardsView = () => (
        <motion.div
            variants={containerVariants} initial="hidden" animate="visible"
            className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:hidden sm:gap-5"
        >
            {schools.map((school) => (
                ...
                <TableBody>
                    {schools.map((school) => (
                        <TableRow key={school.id}>
                            ...
                            <TableCell>
                                <RouterLink to={`/schools-explorer/${school.id}/gradelevels`}>
                                    <Button variant="outline" size="sm">
                                        <DoorOpen className="ml-1.5 h-3.5 w-3.5" />
                                        استكشاف المراحل
                                    </Button>
                                </RouterLink>
                            </TableCell>
