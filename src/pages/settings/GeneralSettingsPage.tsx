...
const GeneralSettingsPage: React.FC = () => {
  ...
  // --- Handlers ---
  const handleSchoolChange = (event: SelectChangeEvent<number | string>) => {
    // Allow string for '' value
    const newSchoolId =
      event.target.value === "" ? null : Number(event.target.value);
    setActiveSchoolId(newSchoolId);

    // Check if the currently selected year is valid for the new school
    const currentYearIsValid = filteredAcademicYears.some(
      (ay) => ay.id === activeAcademicYearId
    );
    if (
      newSchoolId !== null &&
    ...
    if (newSchoolId !== null) {
      enqueueSnackbar("تم تحديد المدرسة النشطة.", {
        variant: "info",
        autoHideDuration: 1500,
      });
    } else {
      enqueueSnackbar("تم إلغاء تحديد المدرسة النشطة.", {
        variant: "info",
        autoHideDuration: 1500,
      });
    }
  ...
  const handleYearChange = (event: SelectChangeEvent<number | string>) => {
    ...
    if (newYearId !== null) {
      enqueueSnackbar("تم تحديد العام الدراسي النشط.", {
        variant: "info",
        autoHideDuration: 1500,
      });
    } else {
      enqueueSnackbar("تم إلغاء تحديد العام الدراسي النشط.", {
        variant: "info",
        autoHideDuration: 1500,
      });
    }
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4, mb: 4 }} dir="rtl">
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
        قم بتحديد المدرسة والعام الدراسي الافتراضي لتسهيل التنقل والعمليات في
        الأقسام الأخرى.
      </Typography>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth required>
              <InputLabel id="active-school-label">
                المدرسة النشطة الافتراضية
              </InputLabel>
              <Select
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth disabled={!activeSchoolId} required>
              <InputLabel id="active-year-label">
                العام الدراسي النشط الافتراضي
              </InputLabel>
              <Select
              ...
              >
                ...
                {!activeSchoolId && (
                  <MenuItem disabled>
                    <em>اختر مدرسة أولاً</em>
                  </MenuItem>
                )}
                {activeSchoolId && filteredAcademicYears.length === 0 && (
                  <MenuItem disabled>
                    <em>لا توجد أعوام لهذه المدرسة</em>
                  </MenuItem>
                )}
                {activeSchoolId &&
          ...
          <Grid item xs={12}>
            <Alert severity="info" variant="outlined" sx={{ mt: 2 }}>
              سيتم استخدام هذه الإعدادات الافتراضية لتحديد المدرسة والعام
              الدراسي مسبقاً في صفحات مثل إدارة التسجيل وإدارة المناهج. يمكنك
              دائماً تغيير التحديد في تلك الصفحات بشكل مؤقت.
            </Alert>
          </Grid>
        </Grid>
      </Paper>
