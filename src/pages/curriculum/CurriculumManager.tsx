...
import {
    Box, Button, Container, Typography, CircularProgress, Alert, Paper, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Stack, FormControl, InputLabel, Select, MenuItem, IconButton, Tooltip, Autocomplete, TextField, Dialog, DialogTitle, DialogContent, DialogActions, DialogContentText
} from '@mui/material';
...
const CurriculumManager: React.FC = () => {
    ...
    const { assignments, loading, error, fetchAssignments, assignSubject, updateTeacherAssignment, unassignSubject, clearAssignments } = useAcademicYearSubjectStore();

    // --- State for Dialogs ---
    const [assignDialogOpen, setAssignDialogOpen] = useState(false);
    const [editTeacherDialogOpen, setEditTeacherDialogOpen] = useState(false);
    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const [currentAssignment, setCurrentAssignment] = useState<AcademicYearSubject | null>(null); // For edit/delete

    // --- State for Assign Dialog Form ---
    const [selectedSubjectId, setSelectedSubjectId] = useState<number | ''>('');
    const [selectedTeacherId, setSelectedTeacherId] = useState<number | null>(null); // Can be null
    ...
    }, [fetchAcademicYears, fetchGradeLevels, fetchSubjects, fetchAllTeachers]);
    ...
    }, [selectedYearId, selectedGradeId, fetchAssignments, clearAssignments]);
    ...
    const handleDeleteConfirm = async () => {
        if (currentAssignment) {
             ...
             if (success) {
                 ...
                 enqueueSnackbar(useAcademicYearSubjectStore.getState().error || 'فشل إلغاء التعيين', { variant: 'error' });
             }
    ...
    return (
        <Container maxWidth="xl" sx={{ mt: 4, mb: 4, direction: 'rtl' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                ...
                <Stack direction="row" spacing={2} alignItems="center">
                     <FormControl sx={{ minWidth: 200 }} size="small">
                         ...
                         <Select
                             labelId="ay-select-label" label="العام الدراسي"
                      ...
                      <FormControl sx={{ minWidth: 200 }} size="small">
                         ...
                         <Select
                             labelId="gl-select-label" label="المرحلة الدراسية"
            ...
            {!loading && !selectedYearId && !selectedGradeId && <Alert severity="info">الرجاء تحديد العام الدراسي والمرحلة لعرض المواد المعينة.</Alert>}
            ...
            {!loading && !error && selectedYearId && selectedGradeId && (
                        ...
                        <Table sx={{ minWidth: 650 }} aria-label="assigned subjects table">
                            ...
                            <TableBody>
                                {assignments.length === 0 && (
                                    <TableRow><TableCell colSpan={4} align="center">لا توجد مواد معينة لهذه المرحلة في هذا العام.</TableCell></TableRow>
                                )}
                                {assignments.map((assignment) => (
                                    <TableRow key={assignment.id} hover>
                                        ...
                                        <TableCell>
                                            {assignment.teacher?.name ?? <Typography component="em" color="text.secondary">غير محدد</Typography>}
                                        </TableCell>
                                        <TableCell align="right">
                                            <Stack direction="row" spacing={0.5} justifyContent="flex-end">
                                                <Tooltip title="تغيير المعلم">
                                                    <IconButton size="small" color="primary" onClick={() => handleOpenEditTeacherDialog(assignment)}>
                                                        <EditIcon fontSize="inherit"/>
                                                    </IconButton>
                                                </Tooltip>
                                                <Tooltip title="إلغاء تعيين المادة">
                                                    <IconButton size="small" color="error" onClick={() => handleOpenDeleteDialog(assignment)}>
             ...
             <Dialog open={assignDialogOpen} onClose={handleCloseAssignDialog} maxWidth="xs" fullWidth>
                 ...
                 <DialogContent>
                     ...
                     <Stack spacing={3} sx={{ pt: 1 }}>
                          <FormControl fullWidth error={!selectedSubjectId && assignLoading}> {/* Basic validation indicator */}
                             <InputLabel id="assign-subject-label">المادة الدراسية *</InputLabel>
                             <Select
                                 labelId="assign-subject-label" label="المادة الدراسية *"
                                 value={selectedSubjectId}
                                 onChange={(e) => setSelectedSubjectId(e.target.value as number)}
                             >
                                  <MenuItem value="" disabled><em>اختر مادة...</em></MenuItem>
                         ...
                         <Autocomplete
                             ...
                             renderInput={(params) => <TextField {...params} label="المعلم المسؤول (اختياري)" />}
                  ...
                  <DialogActions sx={{ px: 3, pb: 2 }}>
                     <Button onClick={handleCloseAssignDialog} color="inherit" disabled={assignLoading}>إلغاء</Button>
                     <Button onClick={handleAssignSubject} variant="contained" color="primary" disabled={assignLoading || !selectedSubjectId}>
             ...
             {/* Edit Teacher Dialog */}
              <Dialog open={editTeacherDialogOpen} onClose={handleCloseEditTeacherDialog} maxWidth="xs" fullWidth>
                 <DialogTitle>تغيير المعلم للمادة "{currentAssignment?.subject?.name}"</DialogTitle>
                 <DialogContent>
                      {assignError && <Alert severity="error" sx={{ mb: 2 }}>{assignError}</Alert>}
                      <Autocomplete
                          ...
                          renderInput={(params) => <TextField {...params} label="المعلم المسؤول (اتركه فارغاً لإلغاء التعيين)" />}
                           isOptionEqualToValue={(option, value) => option.id === value.id}
                          noOptionsText="لا يوجد معلمون"
                      />
                 </DialogContent>
                  <DialogActions sx={{ px: 3, pb: 2 }}>
                     <Button onClick={handleCloseEditTeacherDialog} color="inherit" disabled={assignLoading}>إلغاء</Button>
                     <Button onClick={handleUpdateTeacher} variant="contained" color="primary" disabled={assignLoading}>
            ...
            <Dialog open={deleteDialogOpen} onClose={handleCloseDeleteDialog}>
                ...
                <DialogContent><DialogContentText>هل أنت متأكد من إلغاء تعيين المادة "{currentAssignment?.subject?.name}" من هذه المرحلة الدراسية لهذا العام؟</DialogContentText></DialogContent>
                <DialogActions>
                    <Button onClick={handleCloseDeleteDialog}>إلغاء</Button>
                    <Button onClick={handleDeleteConfirm} color="error">تأكيد الإلغاء</Button>
                </DialogActions>
            </Dialog>
