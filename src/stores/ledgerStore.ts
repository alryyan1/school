...
export const useLedgerStore = create<LedgerState>((set, get) => ({
  ...
  fetchEnrollmentLedger: async (enrollmentId: number, startDate?: string, endDate?: string) => {
    ...
    try {
      ...
      const response = await axios.get(`/student-ledgers/enrollment/${enrollmentId}?${params}`);
      set({ 
      ...
      set({ 
        error: error.response?.data?.message || 'Failed to fetch ledger',
        loading: false 
      });
    }
  },
  ...
  fetchStudentLedger: async (studentId: number, startDate?: string, endDate?: string) => {
    ...
    try {
      ...
      const response = await axios.get(`/student-ledgers/student/${studentId}?${params}`);
      set({ 
      ...
      set({ 
        error: error.response?.data?.message || 'Failed to fetch student ledger',
        loading: false 
      });
    }
  },
  ...
  createLedgerEntry: async (data: CreateLedgerEntryRequest) => {
    ...
    try {
      const response = await axios.post('/student-ledgers', data);
      ...
      set({ 
        error: error.response?.data?.message || 'Failed to create ledger entry',
        loading: false 
      });
      throw error;
    }
  ...
  getLedgerSummary: async (data: LedgerSummaryRequest) => {
    ...
    try {
      const response = await axios.post('/student-ledgers/summary', data);
      set({ loading: false });
      return response.data;
    } catch (error: any) {
      set({ 
        error: error.response?.data?.message || 'Failed to get ledger summary',
        loading: false 
      });
      throw error;
    }
